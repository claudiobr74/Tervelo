...
export function PlateCalculatorScreen() {
  ...
  const assemblies = bar ? listPlateAssemblies({ targetKg, barKg, stock }) : { ok: false as const };
  const exact = bar ? calculatePlates({ targetKg, barKg, stock }) : { ok: false as const };
  const nearest = bar && !exact.ok ? nearestPlateLoads({ targetKg, barKg, stock }) : null;
      ...
      <div className="flex flex-col gap-5 px-6 pb-6">
        ...
        {inventory.error ? (
          <EmptyPanel
            ...
            body="A montagem usa só anilhas gravadas da academia."
          />
        ) : null}
          ...
          <>
            ...
            {primary ? (
              <div className="flex flex-col items-center gap-3 rounded-[var(--radius-xl)] border border-border bg-surface p-4">
                <p className="text-[13px] font-bold uppercase text-foreground">
